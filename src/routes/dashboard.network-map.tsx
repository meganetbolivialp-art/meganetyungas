...
function NetworkMapPage() {
  ...
  const load = async () => {
    const [n, c, l] = await Promise.all([
      ...
      supabase.from("clients").select("id, full_name, latitude, longitude, status").not("latitude", "is", null),
  ...
  const points = useMemo(() => {
    ...
    if (filter.clients) {
      for (const c of clients) {
        ...
        list.push({ id: `c-${c.id}`, lat: +c.latitude, lng: +c.longitude, kind: "client", label: c.full_name, status: st });
      }
    }
